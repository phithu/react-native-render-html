...
export function a(htmlAttribs, children, convertedCSSStyles, passProps) {
  ...
  const onPress = (evt) => onLinkPress && htmlAttribs ?
      onLinkPress(htmlAttribs) : undefined;
  ...
  children = children && children.map((child, index) => {
    ...
    if (rawChild) {
      if (rawChild.parentTag === 'ul') {
        prefix = listsPrefixesRenderers && listsPrefixesRenderers.ul ?
        ...
        prefix = listsPrefixesRenderers && listsPrefixesRenderers.ol ?
            listsPrefixesRenderers.ol(...rendererArgs) :
            (
                <Text
                    style={{ marginRight: 5, fontSize: baseFontSize }}>{index +
                1})</Text>
            );
      }
    }
    return (
        <View key={`list-${nodeIndex}-${index}-${key}`}
...
export function iframe(htmlAttribs, children, convertedCSSStyles, passProps) {
  ...
  const width = attrWidth || classStyleWidth || tagStyleWidth ||
  ...
  return (
      <WebView key={passProps.key} source={{ uri: htmlAttribs.src }}
               style={style}/>
  );
}
...
export function br(htlmAttribs, children, convertedCSSStyles, passProps) {
  return (
      <Text style={{ height: 1.2 * passProps.emSize, flex: 1 }}
            key={passProps.key}>{'\n'}</Text>
  );
}

export function textwrapper(
    htmlAttribs, children, convertedCSSStyles, { key }) {
  return (
